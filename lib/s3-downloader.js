  ...
  return new Promise((resolve, reject) => {
    if (process.env.REPORT_HOOK_URL) {
      request.post(process.env.REPORT_HOOK_URL, {
        form: {
          text: `*${process.env.SERVER_NAME}*: Downloading deploy package from ${url}.`,
        },
      });
    }
    request.get(url)
      .on('response', (response) => {
        if (response.statusCode !== 200) {
          if (process.env.REPORT_HOOK_URL) {
            request.post(process.env.REPORT_HOOK_URL, {
              form: {
                text: `*${process.env.SERVER_NAME}*: Error downloading deploy package. HTTP response was ${response.statusCode} ${response.statusMessage}`,
              },
            });
          }
      ...
      .on('finish', () => {
        if (process.env.REPORT_HOOK_URL) {
          request.post(process.env.REPORT_HOOK_URL, {
            form: {
              text: `*${process.env.SERVER_NAME}*: The deploy is successfully downloaded.`,
            },
          });
        }
        resolve();
  ...
  download() {
    ...
    return getPackage(this.sourceURL, pkgFile).then(() => {
      ...
      if (process.env.REPORT_HOOK_URL) {
        request.post(process.env.REPORT_HOOK_URL, {
          form: {
            text: `*${process.env.SERVER_NAME}*: Extracting deploy package...`,
          },
        });
      }
    ...
    .then(() => {
      ...
      if (process.env.REPORT_HOOK_URL) {
        request.post(process.env.REPORT_HOOK_URL, {
          form: {
            text: `*${process.env.SERVER_NAME}*: Deploy package is extracted successfully.`,
          },
        });
      }
